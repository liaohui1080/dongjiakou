...
var listUserLogin = (function () {
    ...
    var listUserLogin = ['$scope', 'lh_http', 'cfpLoadingBar',
        function ($scope, lh_http, cfpLoadingBar) {
            ...
            $scope.fenye="fenye.html"

            //刷新表格
            $scope.userListShuaxin=function(){
                cfpLoadingBar.start();
                //加载表格数据
                lh_http.get({
                    ...
                    success: function (msg) {
                        ...
                        if (msg.status) {
                            ...
                            $scope.pageSize = 3;
                            $scope.curPage = 0;
                            $scope.pageCount = Math.ceil($scope.userLists.length / $scope.pageSize) - 1
            ...
            $scope.$on('to-userListHangBianji', function (e, msg) {
                ...
                //根据行号绑定数据
                //$scope.userLists[msg.index] = msg.data;

                //console.log("收到 userListHangBianji"); // hello
                //console.log(msg.index); // hello
            ...
            $scope.$on('to-userListHangAdd', function (e, msg) {
                ...
                //$scope.userLists.splice(0,0,msg); //只增加一行, 暂时没用
...
//filter 过滤器
myApp.filter("pageStart", lhFilter.pageStart);
