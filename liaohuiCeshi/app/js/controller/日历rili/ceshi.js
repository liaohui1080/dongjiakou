...
myApp.service('Sample', function Sample() {
    return {
...
myApp.controller('rootController',
    ['$scope', '$log','lh_ajax', function ($scope, $log,lh_ajax) {
        ...
        //console.log(new Date());
        //console.log(new Date(2016, 0, 0, 8, 0, 0));
        //console.log(new Date("January 12,2006 00:19:35"))

        //var jun = moment("2014-06-01T12:00:00Z");
        //var dec = moment("2014-12-01T12:00:00Z");
        //var day = moment.utc("2013-10-06T19:00:00.000Z" , ["MM-DD-YYYY"]);
        //console.log(day)
        //console.log(dec)
// Visual two way binding.
                    ...
                    api.directives.on.new($scope, function(dName, dScope, dElement, dAttrs, dController) {
                        if (dName === 'ganttTask') {
                            dElement.bind('click', function(event) {
                                logTaskEvent2(dScope.task);
                                console.log(dScope.task)
                                $scope.rowEdit=dScope.task.model;
                            });
                        }
                    });
        ...
        var logTaskEvent2 = function (task) {
            ...
            $scope.time=task.model.from
            $scope.editFrom={
                taskName:task.model.name,
                taskFrom:moment(task.model.from).format(),
                taskFromMoment:task.model.from,
                //taskFrom:task.model.from,
                taskTo:moment(task.model.to).format(),
                //taskTo:task.model.to,
        ...
        $scope.editClick=function(){
            ...
            $scope.rowEdit.from=moment($scope.time)
            //$scope.rowEdit.to=moment($scope.time)
            $scope.rowEdit.color=$scope.editFrom.taskColor

            //$scope.rowEdit={
            //    name:$scope.editFrom.taskName,
            //    from:moment($scope.editFrom.taskFrom).format(),
            //    to:moment($scope.editFrom.taskTo).format(),
            //    color:$scope.editFrom.taskColor
            //}
            var dd=moment.utc($scope.editFrom.from);

            console.log(moment($scope.time))
        }
