...
myApp.controller('rootController',
    ['$scope', '$log', 'lh_ajax', '$location','$state', function ($scope, $log, lh_ajax, $location,$state) {
        ...
        $scope.timeType = {
            ...
            upClick: function (o) {
                ...
                $scope.dataQuery({timeScale: $scope.options.scale,
                    ...
                    userType: $scope.userType,
                    from:moment(thisTimeType.updatedTime).format("YYYY-MM-DD"),
                    to:moment(thisTimeType.updatedTime).format("YYYY-MM-DD")
                });
        ...
        //控制当前日历显示规模
        var scaleWacth = $scope.$watchGroup(['options.scale','timeSpans'], function (newVal, oleVal, scope) {
            console.log(newVal);
            console.log(oleVal)
